...
lib.callback.register('union_trucking:server:clockIn', function(source)
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
...
lib.callback.register('union_trucking:server:clockOut', function(source) 
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
    ...
    Player(src).state:set('truckDuty', false, true)
    TriggerClientEvent('union_trucking:client:clearRoutes', src)
    DoNotification(src, 'You have clocked out and cleared all your routes.', 'success')
    return true
end)

lib.callback.register('union_trucking:server:spawnTruck', function(source) 
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
    ...
    TriggerClientEvent('union_trucking:server:spawnTruck', src, NetworkGetNetworkIdFromEntity(vehicle))
    return true
end)

lib.callback.register('union_trucking:server:spawnTrailer', function(source) 
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
...
lib.callback.register('union_trucking:server:chooseRoute', function(source, index) 
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
...
lib.callback.register('union_trucking:server:getRoutes', function(source) 
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
...
lib.callback.register('union_trucking:server:updateRoute', function(source, netid, route)
...
lib.callback.register('union_trucking:server:abortRoute', function(source, index)
    local src = source
    local player = GetPlayer(src)
    local cid = GetPlyIdentifier(player)
