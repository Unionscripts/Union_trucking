...
local function viewRoutes()
    ...
    local routes = lib.callback.await('union_trucking:server:getRoutes', false)
    ...
    for index, data in pairs(routes) do
        ...
        context[#context + 1] = {
            ...
            disabled = isDisabled,
            onSelect = function()
                local choice = lib.callback.await('union_trucking:server:chooseRoute', false, index)
        ...
        if next(activeRoute) and cache.vehicle and IsEntityAttachedToEntity(cache.vehicle, activeTrailer) then
            if IsControlJustPressed(0, 38) and not droppingOff then
                ...
                if lib.progressCircle({
                    ...
                    Wait(500)
                    lib.callback.await('union_trucking:server:updateRoute', false, NetworkGetNetworkIdFromEntity(activeTrailer), activeRoute)
    ...
    pickupZone = lib.points.new({ 
        ...
        onEnter = function()
            if not activeTrailer then
                local success, netid = lib.callback.await('union_trucking:server:spawnTrailer', false)
...
local function spawnPed()
    ...
    targetLocalEntity(truckerPed, {
        { 
            ...
            action = function()
                lib.callback.await('union_trucking:server:clockIn', false)
            end,
        },
        { 
            num = 2,
            ...
            action = function()
                lib.callback.await('union_trucking:server:clockOut', false)
            end,
        },
        {
            num = 3,
        ...
        },
        {
            ...
            action = function()
                ...
                local success, coords = lib.callback.await('union_trucking:server:spawnTruck', false)
        ...
        },
        {
            ...
            action = function()
                local success = lib.callback.await('union_trucking:server:abortRoute', false, activeRoute.index)
                if success then
...
RegisterNetEvent('union_trucking:client:clearRoutes', function()
    if GetInvokingResource() then return end
    cleanupShit()
end)

RegisterNetEvent('union_trucking:server:spawnTruck', function(netid)
